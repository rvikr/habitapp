      ...
      renderItem={({ item }) => {
        ...
        return (
          ...
          >
            ...
            <Text className="text-label-lg font-semibold mb-xs" style={{ color: fg }}>
              {item.name}
            </Text>
            <Text className="text-label-sm" style={{ color: fg + "aa" }}>
              {item.earned ? item.description : item.hintText}
            </Text>
            {!item.earned && item.progressPct > 0 && (
              <View className="h-1 bg-outline-variant rounded-full mt-sm overflow-hidden">
                <View
                  className="h-full rounded-full"
                  style={{ width: `${item.progressPct * 100}%`, backgroundColor: fg }}
                />
              </View>
            )}
          </View>
