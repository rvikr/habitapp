...
-- A row earns leaderboard completion credit only when its logged value reaches
-- the habit target; boolean/targetless habits still count when a row exists.
-- This migration is intentionally ordered after the live service-only
-- leaderboard API migrations and preserves their service_role-only access.
...
as $$
  ...
  completion_totals as (
    select
      ...
      max(cc.completed_on) as last_completion_date
    from completed_credit cc
    where cc.completed_on >= (select start_date from params)
    group by cc.user_id
  ),
  habit_totals as (
    select
  ...
  streak_totals as (
    select
      cc.user_id,
      count(distinct cc.completed_on)::integer as streak
    from completed_credit cc
    where cc.completed_on >= current_date - 60
