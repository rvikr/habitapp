-- Fix consume_ai_quota: modern PostgREST exposes JWT claims as JSON in
-- `request.jwt.claims`, not as per-claim GUCs like `request.jwt.claim.role`.
-- Read both so the function works on old and new platform versions.
