...
private struct LaganWidgetView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: family == .systemSmall ? 5 : 7) {
      HStack {
        Text(entry.snapshot.title).font(.caption.bold())
        Spacer()
        Text(entry.snapshot.levelLabel).font(.caption2.bold()).foregroundStyle(.secondary)
      }
      Text(entry.snapshot.completionLabel)
        .font(family == .systemSmall ? .subheadline.bold() : .headline)
        .lineLimit(2)
      ProgressView(value: Double(entry.snapshot.progressPercent), total: 100).tint(.orange)
      if family != .systemSmall, !entry.snapshot.nextHabitLabel.isEmpty {
        Text(entry.snapshot.nextHabitLabel).font(.caption.bold()).lineLimit(1)
      }
      if let steps = entry.snapshot.steps?.count, entry.snapshot.steps?.status == "available" {
        Text("\(steps.formatted()) steps today").font(.caption2).privacySensitive()
      }
      if let leaderboard = entry.snapshot.leaderboard {
        if leaderboard.status == "ranked", let rank = leaderboard.rank {
          Text("All-time rank #\(rank)").font(.caption2).privacySensitive()
        } else if leaderboard.status == "not_joined", family != .systemSmall {
          Text("Join leaderboard in Lagan").font(.caption2)
        }
      }
      if let message = entry.snapshot.lastAction?.message {
        Text(message).font(.caption2.bold()).foregroundStyle(.green).lineLimit(1)
      }
      Spacer(minLength: 0)
      if #available(iOS 17.0, *), WidgetStore.hasCredential, let next, let habitId = next.id {
        Button(intent: WidgetCheckInIntent(habitId: habitId, habitName: next.name)) {
          Text(next.checkInLabel).frame(maxWidth: .infinity)
        }
        .buttonStyle(.borderedProminent)
        .tint(.orange)
      } else {
        Link(destination: openURL) {
          Text(next?.checkInLabel ?? entry.snapshot.checkInLabel).frame(maxWidth: .infinity)
        }
        .buttonStyle(.borderedProminent)
        .tint(.orange)
      }
