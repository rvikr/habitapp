# Validation Report: Stored web-push endpoints let users steer coach-push outbound requests
...
Evidence: `supabase/migrations/20260605060845_0024_web_push_subscriptions.sql` stores `endpoint` as text, owner RLS permits authenticated users to manage their own rows, and `supabase/functions/coach-push/index.ts` passes that value to `webPush.sendNotification`.
