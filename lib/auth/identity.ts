// Decides how a user re-authenticates before account deletion. Password
// accounts confirm with their password; OAuth-only accounts (Google) have no
// password, so they confirm with a fresh provider sign-in instead. The
// delete-account edge function independently enforces the recent-sign-in
// window server-side, so these helpers only shape the client flow.
