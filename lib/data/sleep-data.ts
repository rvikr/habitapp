...
export async function syncNormalizedSleepEntry(
  source: SleepSource,
  normalized: NormalizedSleepEntry,
): Promise<Result<SleepSyncResult>> {
  ...
  if (!habitResult.ok || !habitResult.data)
    return { ok: false, error: habitResult.error ?? "Could not create sleep habit." };
...
export async function manualLogSleep(
