...
async function setup(page, session) {
  ...
  await page.route("**/*.supabase.co/**", async (route) => {
    ...
    if (path.includes("/rest/v1/rpc/get_completion_dates"))
      return route.fulfill({
        status: 200,
        headers,
        body: completionRows.length ? JSON.stringify([today]) : "[]",
      });
    if (path.endsWith("/rest/v1/rpc/get_completion_stats")) {
      const creditedRows = completionRows.filter((row) => Number(row.value ?? 1) >= habit.target);
      const completionDates = [...new Set(creditedRows.map((row) => row.completed_on))];
      return route.fulfill({
        status: 200,
        headers,
        body: JSON.stringify([
          {
            total_completions: creditedRows.length,
    ...
    if (path.endsWith("/rest/v1/rpc/log_habit_completion_once")) {
      ...
      const increment = Number(payload.p_increment);
      if (
        ...
        return route.fulfill({
          status: 400,
          headers,
          body: JSON.stringify({ message: "invalid exact-once completion payload" }),
        });
      }
      const habitId = payload.p_habit_id || habit.id;
      ...
      exactOnceCalls.push({ operationId, applied, increment });
      if (applied) {
        incrementReceipts.set(operationId, fingerprint);
        const existing = completionRows.find(
          (row) => row.habit_id === habitId && row.completed_on === completedOn,
        );
        const row = {
          id: existing?.id || "detail-completion-1",
          habit_id: habitId,
          user_id: session.user.id,
        ...
        const existingIndex = completionRows.indexOf(existing);
        if (existingIndex >= 0) completionRows.splice(existingIndex, 1, row);
        else completionRows.push(row);
      }
      return route.fulfill({ status: 200, headers, body: JSON.stringify(applied) });
    }
    if (path.endsWith("/rest/v1/rpc/log_habit_completion")) {
      const payload = JSON.parse(req.postData() || "{}");
      const row = {
  ...
  return {
    getCounts: () => ({
      ...
      completionRows,
      exactOnceCalls,
      incrementReceiptCount: incrementReceipts.size,
    }),
  };
}
...
(async () => {
  ...
  if (
    !dashboardText.includes("Drink Water") ||
    !dashboardText.includes("Progress") ||
    !dashboardText.includes("Next")
  ) {
  ...
  if (
    !detailText.includes("Drink Water") ||
    detailText.includes("undefined") ||
    !detailText.includes("0 / 2000 ml")
  ) {
    throw new Error(
      `detail screen did not show initial water progress: ${JSON.stringify(detailText.slice(0, 1200))}`,
    );
  }
  ...
  const result = { counts, snapshots, consoleMessages, pageErrors, requestFailures };
  ...
  if (counts.completionWriteCount !== 1) {
    console.error("expected one completion write from detail quick log");
    process.exit(4);
  }
  if (
    counts.exactOnceCalls.length !== 1 ||
    counts.exactOnceCalls[0].applied !== true ||
    counts.incrementReceiptCount !== 1
  ) {
    console.error("detail quick log did not use one deduplicated exact-once operation");
    process.exit(5);
  }
})().catch((err) => {
  console.error(err);
